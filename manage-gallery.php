			  ...
			  <div class="card">
				<div class="card-header">
				  ...
				  <a href="add-gallery.php">
						<button type="button" class="btn bg-gradient-secondary pull-right">Add New Gallery</button></a>
				</div>
				<!-- /.card-header -->
				<?php 
				$msg = null;
				...
				if(isset($_GET['did'])){
					if($sql = $conn->prepare("DELETE FROM gallery WHERE id='".$_GET['did']."'")){
						$sql->execute();
				...
				<div class="card-body">
				  <table id="example1" class="table table-bordered table-hover">
					...
					<tbody>
						<?php 
						$i=0;
						$category = $conn->query("SELECT * FROM gallery ORDER BY id DESC");
					...
					<tr>
					  ...
					  <td><a href="uploads\<?= $row['image']?>" target="_blank"><?= $row['image']?></a></td>
						...
						<td>
							<a href="edit-gallery.php?cid=<?php echo $row['id']?>" class="btn btn-sm btn-primary mbr-5">
