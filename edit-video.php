...
define ("title", "Video");
define ("page_name", "Edit Video");
$id=$_GET['cid'];
$select = $conn->query("SELECT * FROM video WHERE id='".$id."'");
...
// $ins->execute();
$insert = $conn->prepare("Update video SET name=?, link=? WHERE id='".$id."'");
$insert->bind_param("ss",$title,$link);
...
<div class="row">
	...
	<div class="col-6">
		<div class="form-group">
			<input type="text" class="form-control" id="title" value="<?php echo $data['name'];?>" name="name" placeholder="Enter Title" required>
		</div>
	</div>
</div>
</div>
...
<div class="row">
	...
	<div class="col-6">
		<div class="form-group">
			<input type="text" class="form-control" id="title" name="link" value="<?php echo $data['link'];?>" placeholder="Enter Link" required>
		</div>
	</div>
</div>
</div>
